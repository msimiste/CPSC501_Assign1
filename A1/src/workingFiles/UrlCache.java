...
public class UrlCache {
	...
	/**
	 * Initiates communication with the server, determines if a regular GET or a
	 * conditional GET should be used ie determines if there is a copy of the
	 * file in local cache and if so issues a conditional GET request
	 * 
	 * @param hostName
	 ...
	 * @param path
	 *            - the path/directory where the file exists locally and on the
	 *            server
	 * @param fileName
	 ...
	 * @throws UrlCacheException
	 */
	private void beginDownloadStream(Host host, boolean exists)
			throws UrlCacheException {

		try {

			httpSocket = new Socket(InetAddress.getByName(host.getHostName()),
					host.getPort());
			...
			// locally
			if (exists) {
				long lastMod = getLastModified(host.getHostName()
						+ host.getConcatPath());
	 ...
	 * @throws UrlCacheException
	 */
	private void writeToFile(String path, String fileName, InputStream in)
			throws UrlCacheException {
		...
		// boolean determining if we have reached the end of the header, it is
		...
		try {
			...
			while ((count = in.read(buffer)) > 0) {
				...
				if (!(eoh)) {
					head = new String(buffer, 0, count); // save the header to
															// check it
					int indexOfEoh = head.indexOf("\r\n\r\n"); // save the index
																// of the end of
																// the header
					if (indexOfEoh != -1) {
						offset = indexOfEoh + 4; // this is where the bytes for
													// the file begin
						eoh = true;
						header = new String(buffer, 0, offset);

						// check the header to see if the file has been modified
						// or not
						if (!(inspectHeader())) {
						...
						// if the header is ok create the file, as well as the
						// outputstream
						...
						if ((inspectHeader())) {
							String newPath = path.substring(0,
									path.lastIndexOf("/"));
							File outFile = inOut.makeFileAndDir(newPath,
									fileName);
				...
				// if the inputstream was larger than the bytearray, continue
				// writing to the file
				else {
			...
			// only close the outputstream if it was opened originally, ie if
			// there was an acceptable header
			if (inspectHeader()) {
				out.close();
			}
	...
	/**
	 * 
	 * @return the date in integer format , ie the date is parsed from the
	 *         header and converted to long
	 */
